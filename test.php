...
is($emojiTest->emojiDocomoToUnified($test_docomo),	$test_unified, "DoCoMo -> Unified");
is($emojiTest->emojiKddiToUnified($test_kddi),		$test_unified, "KDDI -> Unified");
is($emojiTest->emojiSoftbankToUnified($test_iphone),	$test_unified, "Softbank -> Unified");
is($emojiTest->emojiGoogleToUnified($test_google),	$test_unified, "Google -> Unified");

echo "#------------------\n";

is($emojiTest->emojiUnifiedToDocomo($test_unified),	$test_docomo,	"Unified -> DoCoMo");
is($emojiTest->emojiUnifiedToKddi($test_unified),	$test_kddi,	"Unified -> KDDI");
is($emojiTest->emojiUnifiedToSoftbank($test_unified),	$test_iphone,	"Unified -> Softbank");
is($emojiTest->emojiUnifiedToGoogle($test_unified),	$test_google,	"Unified -> Google");

echo "#------------------\n";

is($emojiTest->emojiUnifiedToHtml($test_unified),	$test_html,	"Unified -> HTML");
is($emojiTest->emojiHtmlToUnified($test_html),		$test_unified,	"HTML -> Unified");
...
is($emojiTest->emojiGetName(utf8_bytes(0x2600)),	'BLACK SUN WITH RAYS',		"name U+2600");
is($emojiTest->emojiGetName(utf8_bytes(0x26EA)),	'CHURCH',			"name U+26EA");
is($emojiTest->emojiGetName(utf8_bytes(0x1F480)),	'SKULL',			"name U+1F480");
is($emojiTest->emojiGetName(utf8_bytes(0x1F450)),	'OPEN HANDS SIGN',		"name U+1F450");
is($emojiTest->emojiGetName(utf8_bytes(0x1F52B)),	'PISTOL',			"name U+1F52B");
is($emojiTest->emojiGetName(utf8_bytes(0x36).utf8_bytes(0x20E3)),	'KEYCAP 6',	"name U+36 U+20E3");
